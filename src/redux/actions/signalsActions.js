...
export const fetchSignals = () => async(dispatch) =>{
    ...
    const negativeBalanceSignals = data.filter(signal => signal.type === 'negative')
    const checkDocsSignals = data.filter(signal => signal.type === 'docs')
    const cashInconsistencySignals = data.filter(signal => signal.type === 'inconsistency')
